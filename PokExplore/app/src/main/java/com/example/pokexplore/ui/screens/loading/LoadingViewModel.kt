...
class LoadingViewModel(
    ...
    val actions = object : LoadingActions {
        override fun downloadPokemons(onFinished: () -> Unit) = viewModelScope.launch {
            ...
            databaseRepository.upsert(ivysaur)
            databaseRepository.upsert(venusaur)
            databaseRepository.upsert(charmander)
            databaseRepository.upsert(charmeleon)
            databaseRepository.upsert(charizard)
            databaseRepository.upsert(squirtle)
            databaseRepository.upsert(wartortle)
            databaseRepository.upsert(blastoise)
            onFinished()
        }
    }
}